<template>
	<div class="header-container flex flex-wrap items-center justify-between px-20px" :style>
		<div class="flex flex-wrap items-center text-20px">
			<el-icon
				class="mr-7px cursor-pointer"
		...
		<el-dropdown trigger="click" @command="commandCallback">
			<span class="dropdown-title flex flex-wrap items-center">
				<span class="mr-3px">{{ userInfo.info.name }}</span>
				<el-icon>
...
<script lang="ts" setup>
	import { layout, persistenceKeys, project } from '@/config'
	...
	const logout = async () => {
		...
		localStorage.removeItem(persistenceKeys.userInfo)
		router.push('/login')
	}
</script>
